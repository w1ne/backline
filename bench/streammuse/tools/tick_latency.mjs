// Plays a clip (hummed A-minor melody or Am-F-C-G-Am-Dm-Em-Am arpeggio, both from
// bench/voice/synth.ts) into the AMT service the way the browser does (start -> notes at their
// wall-clock onsets -> a cue every half bar) and reports the cue-to-plan latency per commit
// through the relay. Node 20, `ws` from node_modules.
//
...
//
// The CLI below is a thin wrapper over `runClip()`, which other tools (e.g. services/smoke.mjs)
// import to reuse the same clip/latency/scoring logic without shelling out.
...
export function stats(xs) {
  ...
  if (!s.length) return { n: 0, mean: NaN, p50: NaN, p95: NaN, max: NaN };
...
// Runs one clip through the AMT service (via `url`, normally the relay) and returns the raw
// plans/statuses/other messages plus derived stats, without printing or writing anything.
export async function runClip({ url, bars = 16, clip = 'melody', lookaheadBeats = 2, origin = 'https://www.duetai.art' }) {
  const { melody, truth } = buildClip(clip, bars);
  ...
  ws.send(JSON.stringify({ type: 'start', bpm: BPM, key: 'A minor', genre: 'lofi',
    lookaheadBeats, commitBeats: COMMIT_BEATS, listenBeats: LISTEN_BEATS }));
  ...
  const accuracy = truth.length ? atStart.filter((c, i) => c === truth[i]).length / bars : undefined;

  return {
    mode, plans, statuses, others, truth, atStart,
    cuesSent: mode === 'tick' ? cueBeats.length : bars,
    modelPlans, empty, emptyModel,
    cueToPlanAll: stats(rtt),
    cueToPlanModel: stats(modelPlans.map(p => p.rttMs)),
    serverLatency: stats(server),
    sections,
    accuracy,
  };
}
...
async function main() {
  ...
  const r = await runClip({ url: URL, bars: BARS, clip: CLIP, lookaheadBeats: LOOKAHEAD_BEATS });

  console.log(`server ${r.mode === 'tick' ? 'plans per half bar (tick)' : 'plans per bar (no ready)'}; ${BARS} bars at ${BPM} bpm, clip ${CLIP}, lookahead ${LOOKAHEAD_BEATS}`);
  console.log(`cues sent: ${r.cuesSent}, plans: ${r.plans.length} (${r.modelPlans.length} past listen window)`);
  console.log(`cue->plan ms (all):   ${JSON.stringify(r.cueToPlanAll)}`);
  console.log(`cue->plan ms (model): ${JSON.stringify(r.cueToPlanModel)}`);
  console.log(`server latencyMs:     ${JSON.stringify(r.serverLatency)}`);
  console.log(`empty plans: ${r.empty.length} (${r.emptyModel.length} past listen window)`);
  console.log(`notes per plan: ${r.plans.map(p => p.notes?.length ?? 0).join(' ')}`);
  console.log(`chord per plan:  ${r.plans.map(p => p.chord ?? '-').join(' ')}`);
  console.log(`sections seen:   ${r.sections.join(' -> ')}`);
  console.log(`chord at bar start: ${r.atStart.join(' ')}`);
  if (r.truth.length) {
    console.log(`truth:              ${r.truth.join(' ')}`);
    console.log(`bar-start accuracy: ${Math.round(r.accuracy * BARS)}/${BARS} = ${(100 * r.accuracy).toFixed(0)}%`);
  }
  if (r.others.length) console.log('other messages:', JSON.stringify(r.others.slice(0, 5)));
  if (OUT) writeFileSync(OUT, JSON.stringify({ mode: r.mode, plans: r.plans, statuses: r.statuses, others: r.others }, null, 1));
}

// Only run the CLI when this file is invoked directly (not when imported as a module).
