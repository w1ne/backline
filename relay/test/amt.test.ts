...
describe("handleAmt", () => {
  ...
  it("returns 429 on the 121st upgrade from one IP within a minute, and says what the cap is", async () => {
    ...
    expect(await over.text()).toBe("too many requests: at most 120 websocket upgrades per minute per IP");
  });
