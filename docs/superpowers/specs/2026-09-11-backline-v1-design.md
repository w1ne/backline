...
- **Lyria RealTime (Gemini API)** — the any-laptop engine. Our listener steers it: bpm, scale (from the detected key), weighted genre/instrument prompts, mute_drums/mute_bass for toggles, temperature+density from Creativity. Measured 2026-09-11: 3 s to first audio, every control change audible ~2.0–2.6 s later (one bar), `resetContext()` ~0.3 s but cuts audio, so the engine resets on every change and crossfades the seam (150 ms): toggles audible in ~0.5 s. Fixed 2 s chunks, ~3 s playback buffer. One session per jam (reconnects hit 429). Auth is via a Cloudflare Worker relay (`relay/`): the browser signs in with GitHub, the relay checks repo-collaborator access and sets a session cookie, then proxies the Lyria WebSocket with the real Gemini API key attached server-side — no key ever reaches the browser.
