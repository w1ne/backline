...
Rollout: the new client advertises `phraseResponses: true` in its start message. The server defaults this capability off so older open clients cannot receive answers they cannot cancel. Optional phraseFromBeat/phraseToBeat bounds preserve later ordinary backing on the same instrument.
