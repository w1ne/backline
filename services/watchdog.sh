...
if [ "$amt_ok" != "true" ]; then
  ...
  ssh_pod "tmux kill-session -t amt 2>/dev/null; tmux new-session -d -s amt \"bash /opt/backline/services/amt/run-amt.sh ${AMT_PORT} 3 /var/log/amt.log\"" || log "amt restart command failed"
fi
