...
Client → `{"type":"start","bpm":100,"key":"A minor","genre":"lofi","lookaheadBeats":4,"commitBeats":2,"listenBeats":8}`, then `{"type":"notes","notes":[{"beat":12.5,"pitch":64,"dur":0.5,"vel":0.8}]}` as the player plays, `{"type":"bar","bar":3}` at each bar start, `{"type":"set","genre":...,"creativity":...,"instruments":[...],"chord":"Am"}`.
Server → `{"type":"plan","fromBeat":16,"notes":[{"beat":16,"pitch":57,"dur":1,"vel":0.7,"voice":"bass|keys"}]}` per bar, `{"type":"status","latencyMs":230,"tokensPerSec":70}`.

Inside: the model gets the last 16 beats of the human part, samples the accompaniment instrument only (melody instrument masked), monophonic per voice; ~60–300 ms per bar on an L4 (also runs on CPU).
