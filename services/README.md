...
- `chord` / `chordFrom`: the chord the window was voiced against (name as `chordName` produces) and the beat it is in force from (the plan's `fromBeat`). Decided every tick by the melody harmonizer (`harmony.py`) plus the next-chord predictor (`predict.py`, table per `genre` from `start`), stepped over every downbeat between the tick and the window start. `set.chord` from an old client still names the chord until four human notes have been heard; after that the service's decision wins.
- `section`: `intro | groove | lift | breakdown | ending | ended` from `form.py` (a port of src/band/form.ts), ticked once per bar from the latest `set.intensity` (default 0.5) and `set.silenceBeats` (default 0). After `ending` the next bar's plan says `ended` and every plan stays empty until `start` or new `notes` reset the form.
- All three fields are optional: an old client ignores them.
